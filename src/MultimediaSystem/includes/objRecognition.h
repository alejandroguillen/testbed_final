...
#ifndef _CBINDESCRIPTOR_H_
#define _CBINDESCRIPTOR_H_
...
public:
	...
	objRecognition(VisualFeatureExtraction &featExtract, bool isBinary, const int _nDBImages);

	void createDescriptor(const std::string& qImgPath = "", const std::string& dbImgPath = "" );
	...
	std::vector<unsigned int>  rankedQueryDB(std::vector<KeyPoint> &kpts,
											 Mat &features,
													 ...
													 bool do_bin_NNDR = true,
													 int NND_bin_thr = DEFAULT_NND_BIN_THR,
													 float NNDR_ratio = DEFAULT_NNDR_RATIO,
													 bool do_ransac = true,
