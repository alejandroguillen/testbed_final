#include "includes/VisualFeatureExtraction.h"
...
/*
 * Set the detection threshold (if it is possible)
 */
int VisualFeatureExtraction::setDetThreshold(string detName, int thr){

    if(detName.compare("BRISK")==0){
        ((Ptr<BriskFeatureDetector>)detector)->threshold = thr;
        return 0;
    }
    else{
        return -1;
...
/*
 * Set the descriptor with custom parameters
 */
...
int VisualFeatureExtraction::extractKeypoints(Mat &imgGray, vector <KeyPoint> &kpts){
