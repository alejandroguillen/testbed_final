...
void OffloadingManager::addCooperator(Connection* c){
	...
	temp_coop.Pdpx = 350000; //250000
	temp_coop.Pdip = 1000; //1500
...
Mat OffloadingManager::computeLoads(Mat& image){
	...
	//ALEXIS Messages SYNC 19/02
			...
			if(node_manager->node_id == 1)
				out.open("TimesC1.txt", std::ios::app);
			else
				out.open("timesC2.txt", std::ios::app);
			out << current->tm_min << ":" <<current->tm_sec<< " ms: "<<detail_time.tv_usec/1000<<endl;
			out.close();
	//
	*/
	//sortCooperators(); //useful when Equal Split is used
	...
	//ALEXIS assignment 20/02
		int assig=0;
		vector< vector<int> > tmp_optimal_cutvector(cooperators_to_use, vector<int>(cooperators_to_use));
		vector<float> tmp_est_completion_time(cooperators_to_use);

		do{
			if(cooperatorList[0].id!=node_manager->node_id+2) swap(cooperatorList[0],cooperatorList[1]);
			//if(assig==1) swap(cooperatorList[0],cooperatorList[1]);
			...
			tmp_optimal_cutvector[assig]=loadbalancing.getCutVector();
			tmp_est_completion_time[assig]= loadbalancing.getCompletionTime();
			cout << "estimated completionTime " << assig+1 <<" is: " << tmp_est_completion_time[assig] << " with first Ctcoef of "<<cooperatorList[0].id<< " = " << cooperatorList[0].Ctcoef <<endl;
			assig++;
			//c.clear();
			//p.clear();
			//alphad.clear();
		}while(assig<cooperators_to_use-1); //CHANGED!!! simulation lp
		//int position = distance(tmp_est_completion_time.begin(), min_element(tmp_est_completion_time.begin(),tmp_est_completion_time.end()));
		//loadbalancing.setCutVector(tmp_optimal_cutvector[position]);
		//if(position==0 && cooperators_to_use==2) swap(cooperatorList[0],cooperatorList[1]);

		//cout << "min completionTime: " << tmp_est_completion_time[position] << " at assignment " << position+1 << " with first cut at "<< tmp_optimal_cutvector[position][0] << " with first Ctcoef of "<<cooperatorList[0].id<< " = " << cooperatorList[0].Ctcoef<< endl;
	//

	lpsolveTime = (getTickCount()-lpsolveTime)/getTickFrequency();
	std::cerr << "lpsolveTime = " << lpsolveTime << "sec\n";
	vector<int> cutvector = tmp_optimal_cutvector[0]; //CHANGED!!! simulation lp
...
void OffloadingManager::estimate_parameters(cooperator* coop, int i) {
	...
	int ret = coop->processing_speed_estimator->AddObservation(coop->detTime, coop->descTime, coop->mergeTime, coop->Npixelsprocess, coop->Nkeypoints);
	if(ret==0){
		coop->Pdpx = coop->processing_speed_estimator->getPdpx();
		coop->Pdip = coop->processing_speed_estimator->getPdip();
		coop->Pm = coop->processing_speed_estimator->getPm();
		if(coop->Nkeypoints > 0) coop->Pe = coop->processing_speed_estimator->getPe();
	}
	//alpha_d
	double alpha_d = coop->processing_time_coef->setAlphad(coop->Pdpx, coop->Pdip, coop->Pe, coop->Pm);
	coop->alpha_d = alpha_d;

	//Transmission time Coef
	coop->transmission_time_coef->AddObservation(coop->txTime, coop->Npixelssubslices);
	...
	coop->processing_time_coef->AddObservation(coop->processingTime, coop->Npixelsprocess, coop->Nkeypoints, alpha_d);
	...
	std::cerr << "estimate_processing_parameters: Pdpx=" << coop->Pdpx << "\tPdip=" << coop->Pdip << "\tPe=" << coop->Pe << "\tPm" << coop->Pm << "\n";
	...
	std::cerr << "txTime: " << coop->txTime << "\testimated Ctcoef: " << coop->Ctcoef << "sec/pixel" << std::endl;
	...
	std::cerr << "idleTime + txTime + processingTime = " << coop->idleTime + coop->txTime + coop->processingTime  << std::endl;
		...
		out << coop->Ptcoef << " " << coop->Ctcoef << " " << coopbandwitdh << " "  << coop->Npixels << " " << coop->Npixelssubslices << " " << coop->Npixelsprocess << " " << coop->Nkeypoints << " Tidle: " << coop->idleTime << " Tx: " << coop->txTime << " Tm: " << coop->mergeTime << " Tp: " << coop->processingTime << " Tc: "<< coop->completionTime << " alpha: " << alpha_d << std::endl;
		out.close();
...
void OffloadingManager::addKeypointsAndFeatures(vector<KeyPoint>& kpts,Mat& features, Connection* cn,
		double detTime, double descTime, double mergeTime, double processingTime, int Npixelsprocess){
	...
	if(cn){
		for(int i=0;i<cooperatorList.size();i++){
			if(cn == cooperatorList[i].connection){
				...
				cooperatorList[i].mergeTime = mergeTime;
				//cooperatorList[i].fencTime = fencTime;
				cooperatorList[i].processingTime = processingTime;
				cooperatorList[i].Npixelsprocess = Npixelsprocess;
				...
				if(cooperators_to_use==1){
					...
					for(int j=0;j<cooperatorList.size();j++){
						if(cn != cooperatorList[j].connection){
							out << cooperatorList[j].Ptcoef << " " << cooperatorList[j].Ctcoef << " " << 0 << " " << 0 << " "<< 0 << " " << 0 << " " << 0 << " Tidle: " << 0 << " Tx: " << 0 << " Tm: " << 0 << " Tp: " << 0 << " Tc: "<< 0 << " alpha: " <<cooperatorList[j]. alpha_d << std::endl;
						}
					}
...
void OffloadingManager::transmitNextSlice(int i){
	...
	param[0] = CV_IMWRITE_JPEG_QUALITY;
	param[1] = 70;
	cooperatorList[i].Npixelssubslices = 0;
	...
	for(int z=0;z<sub_slices_total;z++){
		...
		double enc_time = getTickCount();
		if(COMPRESS_IMAGE == 1){
			imencode(".jpg",cooperatorSlice[z].image_slice,bitstream,param);
		}else{
			imencode(".bmp", cooperatorSlice[z].image_slice,bitstream);
		}
		enc_time = (getTickCount()-enc_time)/getTickFrequency();
		Coordinate_t top_left;
		top_left.xCoordinate = cooperatorSlice[z].col_offset;
		top_left.yCoordinate = 0;
		
		cooperatorList[i].Npixelssubslices +=cooperatorSlice[z].Npixels;
